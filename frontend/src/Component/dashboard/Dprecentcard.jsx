...
// This component is now wired to show Intent progress.
...
const PercentageCard = () => {
 ...
 const chatId =localStorage.getItem("Ichat_id") || localStorage.getItem("chatId");

  // Fetch percentage from Intent progress API
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        // Fetch the full intent details to calculate progress
        const res = await axios.get(`${API_BASE}/api/session/intent/${chatId}`, {
          headers: { "x-api-key": API_KEY },
        });

        const progressSteps = res.data?.data?.progress || [];
        const completedSteps = progressSteps.filter(
          (step) => step.status === "completed" || step.status === true
        ).length;
        const totalSteps = progressSteps.length;

        const percentage = totalSteps > 0 ? Math.round((completedSteps / totalSteps) * 100) : 0;

        console.log(`Fetched Intent progress: ${completedSteps}/${totalSteps} steps completed (${percentage}%)`);
        setTargetPercentage(percentage);
      } catch (error) {
        console.error("Failed to fetch Intent progress percentage:", error);
      }
    };
